...
class RosenbrockModel(Model):
    ...
    def replicate(self, rng_list):
        ...
        for i in range(self.dim-1):
            noise = noise_rng.normalvariate(1,0.1)
            fn_eval_at_x += 100*(x[i+1]-noise*x[i]**2)**2 + (noise*x[i]-1)**2
...
class RosenbrockProblem(Problem):
    ...
    base.Problem
    """
    def __init__(self, name="RSBR-1", fixed_factors=None, model_fixed_factors=None):
        if fixed_factors is None:
            fixed_factors = {}
        ...
        self.model = RosenbrockModel(self.model_fixed_factors)
