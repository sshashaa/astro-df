...
class ASTRODFDH_problem(Solver):
    """The ASTRO-DF solver.
    ...
    base.Solver
    """
    def __init__(self, name="ASTRODFDH_problem", fixed_factors=None):
        ...
        while True:
            ...
            for i in range(2 * problem.dim + 1):
                ...
                # if iteration k-1 is unsuccessful, we can reuse the replications for x_k
                    ...
                    # adaptive sampling
                    while True:
                        if sample_size >= self.get_stopping_time(k, sig2, delta_k, kappa, problem.dim) or sample_size >= 0.005*budget or expended_budget >= budget:
                            break
    ...
    def iterate(self, k, delta_k, delta_max, problem, visited_pts_list, new_x, expended_budget, budget_limit, recommended_solns, intermediate_budgets, kappa, new_solution):
        ...
        while True:
            ...
            if sample_size >= self.get_stopping_time(k, sig2, delta_k, kappa, problem.dim) or sample_size >= 0.005*budget_limit or expended_budget >= budget_limit:
                break
    ...
    def solve(self, problem):
        ...
        while k < 100:
            record_solution = self.create_new_solution(tuple(new_x), problem)
            problem.simulate(record_solution, 100)
            #print(record_solution.objectives_mean[0])
            gradient_list.append(norm_grad)
            delta_list.append(delta_k)
            function_list.append(record_solution.objectives_mean[0])
        ...
        if problem.name == 'SAN-1':
            file_name_delta = 'delta_san_dh.npy'
            file_name_function = 'function_san_dh.npy'
            file_name_budget = 'budget_san_dh.npy'
        else:
            file_name_delta = 'delta_rsbr_dh.npy'
            file_name_function = 'function_rsbr_dh.npy'
            file_name_budget = 'budget_rsbr_dh.npy'
